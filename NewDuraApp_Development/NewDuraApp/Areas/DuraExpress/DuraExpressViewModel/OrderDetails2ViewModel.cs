...
namespace NewDuraApp.Areas.DuraExpress.DuraExpressViewModel
{
	...
	public class OrderDetails2ViewModel : AppBaseViewModel
	{
		...
		{
			//ShowAlert("We are working with google for tracking link.Backend Team work is inProgress.");
			//await Browser.OpenAsync(GetOrderData.driverlocationlink, new BrowserLaunchOptions {
				...
				try {
					...
					if (result?.ResultType == ResultType.Ok && result?.Data?.status == 200) {
						if (result?.Data?.data != null) {
							// GetOrderData = new GetOrderDetailsModel();
